/**
 * 
 * Build process for angular / couch-potato / require
 * 
 * 1.Convert all templates to JS then move and uglify all javascript files (maintaining folders) to ./dist
 * 2.write script references for all js files to index.html
 * 
 * 
 */
...
configuration.applicationJavascript = ['!app/requireConfig.js', 'app/**/*.js'];
...
gulp.task('angular:templates', function(callback){
    ...
    gulp.src(['app/**/*.html'])
        .pipe(angularTemplateCache('templates.min.js', {module:'intendant', root:'app'}))
        .pipe(uglify())
...
gulp.task('html:index', ['javascript:vendor', 'javascript:build', 'angular:templates', 'css:build', 'css:vendor', 'assets'], function(){
    ...
    var target = gulp.src('index.html'),
        ...
        appScripts = gulp.src(configuration.applicationJavascript),
        vendorScripts = gulp.src(mainBowerFiles({filter:lessRegEx})),
        appStyles = gulp.src(mainBowerFiles({
            filter:(/.*\.css$/i),
            overrides: {
                bootstrap: {
                    main: [
